  ...
  <div>
      {data.map((hero) => {
        return (
          <div >
            <h1>{hero.name}</h1>
            <h3>Вселенная: {hero.universe}</h3>
            <h3>Альтер эго: {hero.alterego}</h3>
            <h3>Род деятельности: {hero.occupation}</h3>
            <h3>Друзья: {hero.friends}</h3>
            <h3>Суперсила: {hero.superpowers}</h3>
            <h3>Информация: {hero.info}</h3>
            <image src={hero.url} alt={hero.name}></image>
            <hr />
          </div>
        );
      })}
